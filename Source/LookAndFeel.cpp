...
void CELookAndFeel::setFontAlt(Font f)
{
...
//==============================================================================
class CELookAndFeel::OSXWindowButton : public Button
{
public:
...
Button* CELookAndFeel::createDocumentWindowButton(int buttonType)
{
	Path shape;
...
void CELookAndFeel::drawLabel(Graphics& g, Label& label)
{
...
void CELookAndFeel::drawTooltip(Graphics& g, const String& text, int width, int height)
{
...
void CELookAndFeel::drawToggleButton(Graphics& g, ToggleButton& button,
...
void CELookAndFeel::drawRoundThumb (Graphics& g, const float x, const float y,
...
void CELookAndFeel::drawButtonBackground (Graphics& g, Button& button, const Colour& backgroundColour, bool isMouseOverButton, bool isButtonDown)
{
...
//	Colour baseColour(mainColour.withMultipliedSaturation(button.hasKeyboardFocus(true) ? 1.3f : 0.9f).withMultipliedAlpha (button.isEnabled() ? 0.9f : 0.5f));
    ...
    if (width > 0 && height > 0)
    {
        ...
        Path outline;
		if (button.getName().compare("openChannelToggleButton2") == 0)
			outline.addEllipse(0.f + spacing, 0.f + spacing, width - (spacing*2.f), height - (spacing*2.f));
		else
		{
...

		{
			if (button.getName().compare("openChannelToggleButton2") != 0 && 
				...
				(button.getName().compare("mainButton") != 0) &&
...
void CELookAndFeel::drawTickBox(Graphics& g, Component& component,
                    ...
                    bool /*isButtonDown*/)
{
/*	if (component.getName().compare("openChannelToggleButton") == 0)
	{
		if (ticked)
			g.setColour(Colours::green);
		else g.setColour(Colours::whitesmoke);

		jassert(burstExtensionsLogo_drawable != 0);
		if (burstExtensionsLogo_drawable != 0)
		{
			burstExtensionsLogo_drawable->replaceColour(Colour(0xff00569c), (ticked ? Colours::green : Colours::whitesmoke));
				
			burstExtensionsLogo_drawable->drawWithin(g, Rectangle<float>(x, y, w, h),
...
void CELookAndFeel::drawLinearSliderThumb (Graphics& g, int x, int y, int width, int height,
...
void CELookAndFeel::drawLinearSlider (Graphics& g, int x, int y, int width, int height,
                        float sliderPos, float minSliderPos, float maxSliderPos,
                        const Slider::SliderStyle style, Slider& slider)
{
...
//==============================================================================
void CELookAndFeel::drawTableHeaderBackground (Graphics& g, TableHeaderComponent& header)
{
...
void CELookAndFeel::drawTableHeaderColumn (Graphics& g, const String& columnName, int /*columnId*/,
...
void CELookAndFeel::fillTextEditorBackground(Graphics &g, int width, int height, TextEditor &textEditor)
{
...
void CELookAndFeel::drawAlertBox(Graphics& g, AlertWindow& alert, const Rectangle<int>& textArea, TextLayout& textLayout)
{
...
void CELookAndFeel::drawProgressBardrawProgressBar(Graphics& g, ProgressBar& progressBar,
...
void CELookAndFeel::drawTabButton(TabBarButton& button, Graphics& g, bool isMouseOver, bool isMouseDown)
{
	...
	g.drawRect(activeArea.reduced(1), 2);
